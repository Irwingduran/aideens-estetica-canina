...
export function LeadCaptureCard({ onSubmit, defaultPhone, hidePhone }: LeadCaptureCardProps) {
  ...
  const phoneValid = hidePhone || whatsapp.replace(/\D/g, "").length >= 10
  const isValid = dogName.trim().length >= 2 && phoneValid
  ...
  async function handleSubmit() {
    ...
    try {
      await onSubmit(dogName.trim(), hidePhone ? (defaultPhone ?? "") : whatsapp.trim())
      setSaved(true)
    } catch {
  ...
  return (
    ...
    >
      <p className="font-sans text-sm font-medium text-cream mb-3">
        {hidePhone ? "Registra a tu perro" : "¿A dónde te mando tu cotización?"}
      </p>
      ...
      <div className="space-y-3">
        <div>
          ...
          <input
            type="text"
            placeholder={hidePhone ? "Rex, Mochi..." : "Rex, Mochi..."}
            value={dogName}
